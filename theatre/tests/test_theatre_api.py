...
from theatre.models import Performance, TheatreHall, Genre, Actor, Play
from theatre.serializers import PlayListSerializer, PlayDetailSerializer

THEATRE_URL = reverse("theatre:plays-list")
...
def image_upload_url(play_id):
    ...
    return reverse("theatre:plays-upload-image", args=[play_id])


def detail_url(play_id):
    return reverse("theatre:plays-detail", args=[play_id])
...
class PlayImageUploadTests(TestCase):
    ...
    def test_image_url_is_shown_on_performance_detail(self):
        ...
        play_id = res.data[0]["play"]
        res = self.client.get(THEATRE_URL + str(play_id) + "/")
